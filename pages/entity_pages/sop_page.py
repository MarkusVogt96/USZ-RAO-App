...
class SOPPage(QWidget):
    def __init__(self, main_window, tumor_type, group_name):
        super().__init__()
        self.main_window = main_window
        self.tumor_type = tumor_type
        self.group_name = group_name
        logging.info(f"Initializing SOPPage for: {self.tumor_type} (Group: {self.group_name})")
    ...
    def go_back(self):
        # Import EntityPage locally (as it's in the same directory now)
        from .EntityPage import EntityPage
        # go_back now goes to the corresponding entity page
        # Need to pass group_name for disambiguation
        current_entity_page_index = self.main_window.find_page_index(EntityPage, entity_name=self.tumor_type, group_name=self.group_name) 
        if current_entity_page_index is not None:
            self.main_window.stacked_widget.setCurrentIndex(current_entity_page_index)
        else:
            logging.warning(f"Could not find EntityPage for {self.tumor_type} (Group: {self.group_name}) when going back. Navigating home.")
            # Fallback to home if the specific entity page isn't found (shouldn't happen ideally)
